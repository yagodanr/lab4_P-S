...
**Explanation:** Since the population variances are known
($\sigma^2 = 1$), we use the **Two-sample Z-test**. For $\alpha=0.05$,
...
nx <- length(x)
ny <- length(y)
se <- sqrt((sigma_sq/nx) + (sigma_sq/ny))
...
if(p_val_z < 0.05) {
  cat("Conclusion: Reject H0 (Means are different).")
} else {
  cat("Conclusion: Do not reject H0 (Means are effectively equal).")
}
```
...
**Explanation:** We use **Fisher's F-test** to compare variances. The
alternative hypothesis is one-sided ($H_1: \sigma_1^2 > \sigma_2^2$), so
we look for $F > F_{crit}$ (right-tailed test).

```{r}
res_f <- var.test(x, y, ratio = 1, alternative = "greater", conf.level = 0.95)
res_f

if(res_f$p.value < 0.05) {
  cat("Conclusion: Reject H0 (Variance of X is significantly greater).")
} else {
  cat("Conclusion: Do not reject H0 (Variances are not significantly different).")
}
```
...
In this task you’ll practice fitting the regression line to some
real-life features and analyzing the results. The file \texttt{data.csv}
...
(b) Fit a linear regression model using Marks as the dependent variable
    and Study Time as the independent variable. Explain shortly the
    process of deriving the regression equation;

(c) Evaluate the goodness-of-fit for the fitted line;

(d) Suggest a way to test whether the Study Time is significant in
    predicting Marks. Find the corresponding test statistic, specify its
    distribution. Find the $p$-value of the test and make a conclusion;
...
In this plot we observe the data, that seems to prove linear dependance
between time study and marks.
...
$Y(x) = ax + b$that is formula for linear regression model, where $a$
and $b$ are parameters.

We choose $a$ and $b$ to minimize $(y_i - ax_i - b)^2$, where $y_i$ is
observed value

```{r}
regr.line <- lm(Marks ~ time_study, data=data)
plot(data$time_study, data$Marks)
abline(regr.line)
```

#### **c. Evaluate the goodness-of-fit for the fitted line;**<br> d. Suggest a way to test whether the study time is significant in predicting marks. Find the corresponding test statistics, specify its distribution. Find the p-value of the test and make a conclusion;

goodness-of-fit is $R^2$ value.

t-test is used to test whether the study time is significant in
predicting marks. It uses t-statistics with approximately Student
distribution
...
$p$-value = $2*10^{-16}$, which means that with $\alpha > p$-value we
reject $H_0$(time study is not statistically significant in predicting
marks)

#### e. If Alice studies for approximately $8$ hours, what grade can we predict for her?

```{r}
Alice <- data.frame(time_study = 8)
regr.line.Alice <- predict(regr.line, Alice)
regr.line.Alice
```


#### f. Suggest up to three ideas to potentially improve prediction accuracy.
1. Get data about students' attendance, sleep hours and so on to better
2. Normalize data
```{r}
data_norm <- data
data_norm$time_study_norm <- scale(data_norm$time_study)
data_norm$Marks_norm <- scale(data_norm$Marks)
regr.line2 <- lm(Marks_norm ~ time_study_norm + I(time_study_norm^2), data=data_norm)
plot(data_norm$time_study_norm, data_norm$Marks_norm)
abline(regr.line2)
summary(regr.line2)
summary(regr.line)
